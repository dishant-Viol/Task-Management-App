...
const DataTable: React.FC<DataTableProps> = ({ data, columns, setData }) => {
  const [newRow, setNewRow] = useState<RowData>({ name: '', email: '', role: '' }); // Added email to newRow
  ...
  const handleAddClick = () => {
    setNewRow({ name: '', email: '', role: '' }); // Reset the new row data including email
        ...
        <thead className="bg-blue-600 text-white">
          <tr>
            {columns.map((column) => (
              <th
                key={column.accessor}
                className="py-3 px-4 border-b text-left text-sm font-semibold"
              >
                {column.Header}
              </th>
            ))}
